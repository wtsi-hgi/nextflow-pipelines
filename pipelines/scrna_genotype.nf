...
//	.set{ch_samplename_pooled_npooled_location}
    ...
    Channel.fromPath("${baseDir}/../../inputs/n_samples_lanes_23_add1.csv")
	.splitCsv(header: true)
	.map { row -> tuple("${row.lane}", "${row.n_samples}") }
 ...
 //   ch_samplename_pooled_npooled_location.view()
    ...
    ch_samplename_location
	.map { a,b -> [a,file("${b}/filtered_feature_bc_matrix")] }
	.set{ch_cellranger_filtered}

    ch_samplename_location
	.map { a,b -> [a,file("${b}/*.bam"),file("${b}/*.bam.bai"),file("${b}/filtered_feature_bc_matrix/barcodes.tsv.gz")] }
	.set{ch_cellranger_sample_bam_barcodes}

    ch_samplename_location
	.map { a,b -> [a,file("${b}/metrics_summary.csv")] }
	.set{ch_cellranger_metrics_summary}
    
    ch_cellranger_metrics_summary
