...
Channel.fromPath(params.mbv_vcf_gz)
    ...
    .set { ch_mbv_vcf_gz }
Channel.fromPath(params.mbv_vcf_gz_csi)
    .ifEmpty { exit 1, "MBV vcf csi not found: ${params.mbv_vcf_gz_csi}" }
    .set { ch_mbv_vcf_gz_csi }
...
workflow {
	...
	// if(params.run_mbv) { mbv(iget_cram.out[0].take(2), ch_mbv_vcf_gz.collect()) }
