...
for index, row in fofn_countsfiles.iterrows():
    print(row['count_file'].replace('.counts.txt','.subpools.counts.txt'))
    if index == 0:
        concatenated = pd.read_csv(row['count_file'].replace('.counts.txt','.subpools.counts.txt'), sep='\t')
        concatenated['samplename']=row['samplename']
    else:
        to_concatenate = pd.read_csv(row['count_file'].replace('.counts.txt','.subpools.counts.txt'), sep='\t')
        to_concatenate['samplename']=row['samplename']
        concatenated = pd.concat([concatenated, to_concatenate], ignore_index=True)
    print(concatenated.head())
        
count_matrix = concatenated.pivot_table(index=['Guide Sequence','Subpool'], columns='samplename', values='Count').reset_index()       
count_matrix.to_csv(lib_counts + '.subpools.count_matrix.txt', sep='\t', index = False)
