...
##############
# supplier_sample_name rather than sangers_sanger_id plots:
minimal_spreadsheet = read_tsv(minimal_spreadsheet) %>%
    rowwise %>%
    mutate(sanger_sample_id = ifelse(is.na(sanger_sample_id),
                                     supplier_sample_name, sanger_sample_id)) %>%
    ungroup %>%
    filter(!is.na(sanger_sample_id)) %>%
    mutate(supplier_sample_name_original = supplier_sample_name)

print('supplier_sample_name_original column')
print(minimal_spreadsheet$supplier_sample_name_original)
print('supplier_sample_name_original column')
...
minimal_spreadsheet %<>% 
       left_join(csv_files) %>%
       select(-csv_file)
print('left_join csv_files applied')

to_save = minimal_spreadsheet %>%
          dplyr::rename(supplier_sample_name = supplier_sample_name_original) %>%
          select(-supplier_sample_name)
write_tsv(to_save, "samples_metainfo_supplier_name.tsv")
write.xlsx(as.data.frame(to_save),"samples_metainfo_supplier_name.xlsx")
...
out_file <- "samples_status_pre_pipelines.pdf"
...
out_file <- "samples_status_dropped_pre_pipelines.pdf"
pdf(file = out_file, height = 4, width = 5.5)
print(plt2)
dev.off()
