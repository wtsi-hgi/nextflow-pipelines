...
echo starting bsub
rm -f hs_err_pid* && rm -f timeline* && rm -f trace* && rm -rf report* && rm -f bsub.o && rm -f bsub.e && rm -f .nextflow.log && bsub -G hgi -R'select[mem>8000] rusage[mem=8000] span[hosts=1]' -M 8000 -n 1 -o bsub.o -e bsub.e -q yesterday ./nextflow-pipelines/bin/variant_calling/nextflow.sh > bjob.id
echo finished bsub
cat bjob.id
